...
# Executive Support Skill

> **Tier** 2 — NoblePort Operations · **Owner role** Executive / Owner — final authority: Michael F. O'Rourke
> **Powered by** Stephanie.ai (`StephanieAgent`) · `/api/ops-brief`, `/api/v1/dashboard`, `/api/governance` · **Last reviewed** 2026-07-01
>
> *Formerly `06-stephanie-executive`. Named by function (executive support) so the
> role survives changes in who or what fills it; the executive agent that powers
> it today is **Stephanie.ai**.*
...
## When NOT to use
- Executing money/permit/contract actions — this layer is **advisory only**.
...
## Related Skills
- **Input** ← all tiers (telemetry from every skill's system counterpart)
- **Output** → owner decisions; `14-recruiting` (capacity → hiring needs)
- **Dependency** ⋈ `09-change-orders` · `10-payment-node` (the governance gates it monitors)
...
- No recommendation implies an action this layer is not authorized to take.
